...
class Controller:
    def __init__(self, maze):
        ...
        # PDDL parameters
        self.problem_name = "CAJAS-4"
        self.domain_name = "CAJAS_domain"
        self.objects = "ARANDELA TUERCA CLAVO TORNILLO"
        self.problem_file = "CAJAS_problem.pddl"
    ...
    def write_pddl_problem_file(self, rectangles_init, rectangles_goal):
        ...
        init_string = f"""(MANOLIBRE)
(LIBRE {init_order[0][1]})
(ENMESA {init_order[3][1]})
(SOBRE {init_order[0][1]} {init_order[1][1]})
(SOBRE {init_order[1][1]} {init_order[2][1]})
(SOBRE {init_order[2][1]} {init_order[3][1]})
"""

        goal_string = f"""(MANOLIBRE)
(LIBRE {goal_order[0][1]})
(ENMESA {goal_order[3][1]})
(SOBRE {goal_order[0][1]} {goal_order[1][1]})
(SOBRE {goal_order[1][1]} {goal_order[2][1]})
(SOBRE {goal_order[2][1]} {goal_order[3][1]})
"""
        #print(init_string)
        #print(goal_string)
...
"""
    ...
    def search_ppdl_solution(self, rectangles_init, rectangles_goal):
        ...
        def run_planner(problem_file, domain_file, search_algorithm="astar(blind())", ):
            """
        ...
        domain_file = self.domain_name + ".pddl"
        print(domain_file)
        planning_result = run_planner(problem_file=filename, search_algorithm="astar(lmcut())", domain_file=domain_file)
        return self.extract_actions(planning_result)

    @staticmethod
    def extract_actions(log):
        # Define action keywords to look for
        action_keywords = ["desapilar", "bajar", "agarrar", "apilar"]
        actions = []
    ...
    def launch_algorithm(self, algorithm, image):
        if algorithm == "K-Means":
            ...
            vote_group, vote_result, confidence, hull_image, features, scaled_features, features_names = kmeans.launch_kmeans()

            features_dict = dict(zip(features_names, features))
            scaled_features_names = [f'Scaled_{feature}' for feature in features_names]
            ...
            vote_result, confidence, hull_image, features, scaled_features, features_names = knn.launch_knn()
